...
class decisionTree:
    ...
    def split(self, n, points, pred_vals, pred_opps, depth, x, y, predictions):
        i = points[0][1]
        j = points[0][0]
        prediction = 0
        if x[i,j,n] == 1:
            if depth>1:
                prediction = self.split(n, points[1:], pred_vals, pred_opps, depth-1, x, y, predictions)
            else:
                prediction = pred_vals[points[0]]
        else:
            if depth>1:
                prediction = self.split(n, points[2:], pred_vals, pred_opps, depth-1, x, y, predictions)
            else:
                prediction = pred_opps[points[0]]
        return prediction
    ...
    def applyDT(self, points, pred_vals, depth, x, y):
        ...
        for n in range(200):
            predictions[n] = self.split(n, points, pred_vals, pred_opps, depth, x, y, predictions)
        return predictions
